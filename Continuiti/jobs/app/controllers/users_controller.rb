...
class UsersController < BaseController
  ...
  def rate
    @user = current_user
    # user rating logic for job feedback 
  ...
  def show
    init_gmap
    @skills                     = UserSkill.all(:conditions => "user_id = '#{@user.id}'", :order => "created_at DESC")
    @certifications             = Certification.all(:conditions => "user_id = '#{@user.id}'", :order => "created_at DESC")
    @interests                  = Interest.all(:conditions => "user_id = '#{@user.id}'", :order => "created_at DESC")
    @memberships                = Membership.all(:conditions => "user_id = '#{@user.id}'", :order => "created_at DESC")
    @endorsements               = Endorsement.all(:conditions => "user_id = '#{@user.id}'", :order => "created_at DESC")
    @my_badges                  = AwardUser.all(:conditions => "user_id = '#{@user.id}'", :limit => 5, :order => "created_at DESC")
    ...
    @educations = Education.find(:all, :conditions => ["user_id =? ", @user.id], :order => "created_at DESC")
    @experiences = Experience.find(:all, :conditions => ["user_id =? ", @user.id], :order => "created_at DESC")
    recent_experience = Experience.all(:conditions => "user_id = '#{@user.id}'", :order => "end_year DESC", :limit => 1)
    @my_recent_experience = if recent_experience.blank?
      Experience.all(:conditions => "user_id = '#{@user.id}' AND current = '1'", :limit => 1, :order => "created_at DESC")
    else
      recent_experience
    end
    @my_recent_education = Education.all(:conditions => "user_id = '#{@user.id}'", :order => "end_year DESC", :limit => 1)
    @my_recent_certification = Certification.all(:conditions => "user_id = '#{@user.id}'", :select => "title", :order => "award_date DESC")
    @my_current_skill = UserSkill.all(:conditions => "user_skills.user_id = '#{@user.id}'", :joins => :skill, :select => "skills.name as title", :order => "user_skills.year_period DESC")
    @my_user_description = UserDescription.find_by_user_id(@user.id)
    @my_opportunity_preferences = UserOpportunityPreference.find_all_by_user_id(@user.id)
    @my_general_availabilities = UserGeneralAvailability.find_all_by_user_id(@user.id)
  ...
  def new
    ...
    @invitation = Invitation.find_by_token params[:inviter_code] if params[:inviter_code]

    session[:invite_code] = params[:inviter_code] if params[:inviter_code]
  ...
  def create
    ...
    if (!AppConfig.require_captcha_on_signup || verify_recaptcha(@user)) && @user.save
      ...
      if session[:inviter_code]
        invitation = Invitation.find_by_token session[:inviter_code]
        Referral.create(:referrer_id => invitation.user.id, :referred_id => current_user.id)
      ...
      unless params[:opportunity_preference].blank?
        ...
        params[:opportunity_preference].each do |key, val|
          opportunity_preference_values << {:user_id => @user.id, :opportunity_preference_id => key}
        end
  ...
  def update
    #XXX this method is fucking horrific, needs refactoring
    @user.attributes      = params[:user]
    @metro_areas, @states = setup_locations_for(@user)
    @user.locations.build(params[:user][:location_attributes]) unless params[:user][:location_attributes].nil?
    ...
    if @user.save!
      ...
      if (!params[:education].nil? && !params[:education][:degree].blank? && !params[:education][:major].blank? && !params[:education][:organization].blank?)
        edustuff = params[:education].merge(:user_id => current_user.id)
        Education.create(edustuff)
      end
  ...
  def init_gmap
    ...
    @user.locations.each do |location|
      ...
      @map[:location => location.location_name] = GMap.new("map_#{location.location_name}")
      @map[:location => location.location_name].control_init(:large_map => true, :map_type => true)
      @map[:location => location.location_name].center_zoom_init(coordinates,15)
      @map[:location => location.location_name].overlay_init(GMarker.new(coordinates,:title => "Your location"))
    end
