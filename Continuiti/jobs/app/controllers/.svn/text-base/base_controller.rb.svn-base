...
class BaseController < ApplicationController
  ...
  helper_method :commentable_url, :user_display_name
  ...
  # end  
  def cache_action?
    !logged_in? && controller_name.eql?('base') && params[:format].blank? && [:gender, :age_group_id, :style, :occasion].reject{|prm| params[prm].blank?}.length==0
  end 
  ...
  def site_index
    ...
    respond_to do |format|
      format.html do 
        if current_user.nil?
          users = User.find(:all, :limit => 50, :conditions => ["avatar_id IS NOT NULL"])
          @all_avatars = users.map do |u|
            u.avatar.public_filename(:thumb) 
          end
          render :action => 'landing', :layout => 'minimal'
        else
  ...
  def user_display_name(user)
    ...
    return user.first_name+" "+user.last_name unless user.first_name.blank? && user.last_name.blank?
    return user.login
  end

  private
    def admin_required
      current_user && current_user.admin? ? true : true
    end
