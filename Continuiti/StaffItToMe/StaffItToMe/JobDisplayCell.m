...
- (id)initWithFrame:(CGRect)frame pictureURL:(NSString*)picture_url name:(NSString*)the_name description:(NSString*)the_description detail:(NSString*)the_detail
{
    ...
    if (self) {
        ...
        if (the_name != nil && 
            the_name != [NSNull null])
        {
            job_one_name.text = the_name;
        }
        else
        {
            job_one_name.text = @"Not available";
        }
        [self addSubview:job_one_name];
        
        job_one_description                 = [[UILabel alloc] initWithFrame:CGRectMake(job_one_name.frame.origin.x, job_one_name.frame.origin.y + job_one_name.frame.size.height - 11, 140, 30)];
        ...
        if (the_description != nil && 
            the_description != [NSNull null])
        {
            job_one_description.text = the_description;
        }
        else
        {
            job_one_description.text = @"Not Available";
        }
