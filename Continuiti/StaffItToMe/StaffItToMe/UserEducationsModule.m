...
-(id)initWithArray:(NSArray*)the_experiences
{
    if ((self = [super init]))
    {
        ...
        else
        {
            ...
            {
                int from_year;
                NSMutableString *detail_text;
                if ([[the_experiences objectAtIndex:i] objectForKey:@"end_year"] != nil && [[the_experiences objectAtIndex:i] objectForKey:@"end_year"] != [NSNull null])
                {
                    from_year   = [[[the_experiences objectAtIndex:i] objectForKey:@"end_year"] intValue];   
                    detail_text = [[NSMutableString alloc] initWithString:@"End Year: "];
                    [detail_text appendString:[NSString stringWithFormat:@"%d", from_year]];
                }
                else
                {
                    from_year = 0;
                    detail_text = [[NSMutableString alloc] initWithString:@"NA"];
                }
